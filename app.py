...
for index, message in enumerate(st.session_state.messages):
    ...
    with st.chat_message(message["role"]):
        if message["type"] == "text":
            ...
            # --------------------------------------------------
            # Remove accidental HTML generated by the LLM
            # --------------------------------------------------
            raw_content = re.sub(r"<[^>]+>", "", raw_content)

            # Escape anything that is still HTML-sensitive
            ...
            if contains_kannada(raw_content):
                content_style = (
                    'color:#E2F1FF;'
                    'font-family:"Nirmala UI","Noto Sans Kannada","Arial Unicode MS",sans-serif;'
                    'font-size:17px;'
                    'line-height:1.7;'
